...
const appendEnv = async () => {
	...
	await readFile(`.env`, `utf8`, async (err, data) => {
		if (data && data.includes(`@strapi-community/herokufy variables `)) {
			await envUpdate(config.env);
			return;
		}

		writeLine(`# @strapi-community/herokufy variables \n`);
		...
		spinner.stopAndPersist({
			symbol: `🕵️`,
			text: `  Added ${chalk.bold.blue(`herokufy`)} variables in ${chalk.yellow(
				`.env`
			)} \n`
		});
	});
